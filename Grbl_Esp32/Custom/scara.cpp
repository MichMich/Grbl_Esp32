...
bool cartesian_to_motors(float* target, plan_line_data_t* pl_data, float* position) {
    ...
    new_target[X_AXIS] = target[X_AXIS];
    new_target[Y_AXIS] = target[Y_AXIS] + target[X_AXIS] / 2;
    new_target[Z_AXIS] = target[Z_AXIS];
    new_target[A_AXIS] = target[A_AXIS];

    return mc_line(new_target, pl_data);
}
...
void motors_to_cartesian(float* cartesian, float* motors, int n_axis) {
    ...
    cartesian[Y_AXIS] = motors[Y_AXIS] - motors[X_AXIS] / 2;
